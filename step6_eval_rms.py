...
parser.add_argument(
    "--output_dir", type=str, default="temp_out_dir"
)  # generated_responses/ ,  generated_summaries/
...
for max_len in max_len_list:
    ...
    for split in range(5):
        ...
        for test_gen_pref_model_name in [args.gen_pref_model_name]:  #
            if args.embed_model_name == "gemma2b":
                embedding_dataset = torch.load(
                    f"/mnt/bn/hsunvolume-lq/Step4_2_testall/EXPembed_model_name_{args.embed_model_name}_dataset_orig_hh-rlhf-{args.task}{args.sft_obj}_gen_pref_model_name_{test_gen_pref_model_name}_n_samples_500_max_len_128_split_{split}_server_alias_lq/{args.embed_model_name}_{test_gen_pref_model_name}_hh-rlhf-{args.task}{args.sft_obj}_{split}_testembedding_total_128.pt"
                )
            elif args.embed_model_name == "gemma7b":
                embedding_dataset = torch.load(
                    f"/mnt/bn/hsunvolume-lq/Step4_2_testall_7b/EXPembed_model_name_{args.embed_model_name}_dataset_orig_hh-rlhf-{args.task}{args.sft_obj}_gen_pref_model_name_{test_gen_pref_model_name}_n_samples_500_max_len_128_split_{split}_server_alias_lq/{args.embed_model_name}_{test_gen_pref_model_name}_hh-rlhf-{args.task}{args.sft_obj}_{split}_testembedding_total_128.pt"
                )
            else:
            ...
            for model_i in range(args.ensemble_number - 1, args.ensemble_number):
                ...
                if args.normal_or_xprompt == "xprompt":
                    ...
                    raise ValueError(
                        f"Invalid normal_or_xprompt: {args.normal_or_xprompt}"
                    )
                model.load_state_dict(torch.load(pre_fix + post_fix))
                model.eval()
                model_i_all_preds = []
                for instance_i in range(embedding_dataset.shape[1]):
                    model_i_all_preds.append(
                        model(torch.tensor(embedding_dataset[:, instance_i]))
                        .detach()
                        .numpy()
                    )
            ...
            for instance_i in range(embedding_dataset.shape[1]):
                ...
                quantile_mean = np.sum(
                    np.asarray(reward_list[instance_i]) <= real_value_mean
                ) / len(reward_list[instance_i])
                quantile_min = np.sum(
                    np.asarray(reward_list[instance_i]) <= real_value_min
                ) / len(reward_list[instance_i])
                quantile_first = np.sum(
                    np.asarray(reward_list[instance_i]) <= real_value_first
                ) / len(reward_list[instance_i])
            ...
            print(
                f"Mean quantile of selected max using MEAN ESB {np.mean(quantile_list_mean)}"
            )
            print(
                f"Mean quantile of selected max using MIN ESB {np.mean(quantile_list_min)}"
            )
            print(
                f"Mean quantile of selected max using FIRST ESB {np.mean(quantile_list_first)}"
            )
            ...
            for instance_i in range(embedding_dataset.shape[1]):
                ...
                correlations.append(
                    spearmanr(
                        model_predictions_mean[instance_i], reward_list[instance_i]
                    ).correlation
                )
            correlations_mean = np.array(correlations)
            print(
                f"Mean Spearman correlation using MEAN ESB {np.mean(correlations_mean[~np.isnan(correlations_mean)])}"
            )
            ...
            for instance_i in range(embedding_dataset.shape[1]):
                ...
                correlations.append(
                    spearmanr(
                        model_predictions_min[instance_i], reward_list[instance_i]
                    ).correlation
                )
            correlations_min = np.array(correlations)
            print(
                f"Mean Spearman correlation using MIN ESB {np.mean(correlations_min[~np.isnan(correlations_min)])}"
            )
            ...
            for instance_i in range(embedding_dataset.shape[1]):
                ...
                correlations.append(
                    spearmanr(
                        model_predictions_first[instance_i], reward_list[instance_i]
                    ).correlation
                )
            correlations_first = np.array(correlations)
            print(
                f"Mean Spearman correlation using FIRST ESB {np.mean(correlations_first[~np.isnan(correlations_first)])}"
            )
            ...
            for repeat_exp in range(100):
                ...
                for N_best in [2, 5, 10, 30, 50, 100, 300, 500]:
                    ...
                    reward_mean = np.array(
                        [
                            effect_reward_list[i][argmax_mean[i]]
                            for i in range(embedding_dataset.shape[1])
                        ]
                    )
                    reward_min = np.array(
                        [
                            effect_reward_list[i][argmax_min[i]]
                            for i in range(embedding_dataset.shape[1])
                        ]
                    )
                    reward_first = np.array(
                        [
                            effect_reward_list[i][argmax_first[i]]
                            for i in range(embedding_dataset.shape[1])
                        ]
                    )
            ...
            plt.fill_between(
                range(8),
                reward_mean_list.mean(0) - reward_mean_list.std(0),
                reward_mean_list.mean(0) + reward_mean_list.std(0),
                alpha=0.3,
            )
            ...
            plt.fill_between(
                range(8),
                reward_min_list.mean(0) - reward_min_list.std(0),
                reward_min_list.mean(0) + reward_min_list.std(0),
                alpha=0.3,
            )
            ...
            plt.fill_between(
                range(8),
                reward_first_list.mean(0) - reward_first_list.std(0),
                reward_first_list.mean(0) + reward_first_list.std(0),
                alpha=0.3,
            )
            ...
            results[f"split{split}"][test_gen_pref_model_name] = {
                "correlations_mean": np.mean(
                    correlations_mean[~np.isnan(correlations_mean)]
                ),
                "correlations_min": np.mean(
                    correlations_min[~np.isnan(correlations_min)]
                ),
                "correlations_first": np.mean(
                    correlations_first[~np.isnan(correlations_first)]
                ),
    ...
    # save results
    with open(
        f"{args.output_dir}/MLP_x_or_n_{args.normal_or_xprompt}_{args.rm_objective}_epoch{args.training_epochs}_lr{args.learning_rate}_XPrompt-RESULTS_firstn_{args.consider_first_n}_n_{args.n_sample}_pair_{args.n_pairs}_seed{args.seed}_BON-{args.max_len}_replacement_{args_replacement}.json",
        "w",
    ) as f:
        json.dump(results, f)
