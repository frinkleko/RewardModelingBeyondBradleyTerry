...
ADAPTER_PATH = (
    f"{args.output_dir}/ckpts_SFT/SFT_{args.model_name}_{args.dataset}_lr5e-05_epoch2"
)
...
if "hh-rlhf" in args.eval_dataset:
    ...
    dataset = dataset.select(
        range(split_len * args.split, split_len * (args.split + 1))
    )
    ...
    dataset = load_dataset(
        "csv",
        data_files=f"data/helpsteer_{args.data_class}_prompts_cleaned.csv",
        split="train",
    )
    total_len = len(dataset)
    split_len = int(total_len / 5)
    ...
    dataset = load_dataset(
        "csv",
        data_files=f"data/ultrafeedback_{args.data_class}_prompts_cleaned.csv",
        split="train",
    )
    total_len = len(dataset)
    split_len = int(total_len / 5)
...
sampling_params = SamplingParams(
    temperature=args.temperature, top_p=0.95, max_tokens=args.max_len
)
dtype = get_data_type()
llm = LLM(
    model=MODEL_PATH,
...
for i in tqdm(range(len(dataset))):  # number of demonstrations to create
    ...
    if args.adapter_name is None:
        batch_response = llm.generate(
            [query for _ in range(args.n_samples)], sampling_params
        )
    else:
        batch_response = llm.generate(
            [query for _ in range(args.n_samples)],
            sampling_params,
            lora_request=LoRARequest("lora_adapter_name", 1, ADAPTER_PATH),
        )
