...
def main():
    ...
    parser.add_argument("--teacher_ckpt", type=str, required=True, help="Path to the trained ORM checkpoint")
    ...
    if not os.path.exists(sample_emb_path):
        print(f"Warning: Could not find embeddings at {sample_emb_path}. Using dummy dimension 2048 for script structure.")
        input_dim = 2048
    else:
    ...
    for i in range(10):
        ...
        if not os.path.exists(score_file):
            score_file = os.path.join("temp_out_data", score_file) # check alternate loc
        if not os.path.exists(score_file):
        ...
        with open(score_file, 'r') as f:
            for line_idx, line in enumerate(f):
                ...
                golden_score = data['scores_sorted'] # This is the scalar from the Golden RM
    ...
    X_train, X_test, y_train, y_test = train_test_split(Embeds, Y_calibrated, test_size=0.2, random_state=42)
    ...
    for epoch in range(args.distill_epochs):
        ...
        print(f"Epoch {epoch+1}/{args.distill_epochs}, Loss: {epoch_loss/(len(X_train)/batch_size):.4f}, Val MSE: {val_loss.item():.4f}")
