...
for idx in tqdm(range(len(response_data))):
    ...
    for batch_i in range(total_batch):
        for i in range(10):
            model_gen_answer = extract_model_gen_answer(
                instance[f"response_{batch_i*10 + i}"]
            )
    ...
    with open(
        f"{response_path}_rm{args.eval_rm_name}_{args.split}_maxlen{args.max_len}.json",
        "a+",
    ) as f:
