...
raw_datasets = load_dataset(
    "json", data_files=folder_name + "config.json", split="train"
)
...
for idx in range(1, len(embedding_dataset_total)):
    final_out_embedding = np.concatenate(
        (final_out_embedding, embedding_dataset_total[idx]), axis=1
    )
...
torch.save(
    final_out_embedding,
    args.output_dir
    + f"/{args.embed_model_name}_{args.gen_pref_model_name}_{args.split}_embedding_total_{args.max_len}.pt",
)
