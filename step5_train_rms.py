...
for i in range(10):
    embedding_dataset.append(
        torch.load(
            f"/mnt/bn/hsunvolume-{args.server_alias}/Step4_2_reorg_tensors_all/embed_model_name_{args.embed_model_name}_dataset_orig_hh-rlhf-{args.task}{args.sft_obj}_gen_pref_model_name_{args.gen_pref_model_name}_golden_rm_name_{args.rm_name}_train_test_train_server_alias_lq_embedding_only_rank_{i}.pt"
        ).numpy()
    )
    if "helpful" in args.task:
        dataset = load_dataset(
            "json",
            data_files=f"data_10split/rmmistral7b_{args.gen_pref_model_name}_train_hh-rlhf-{args.task}{args.sft_obj}_database_split{i}.json",
            split="train",
        )
        ...
        dataset = load_dataset(
            "json",
            data_files=f"data_10split/rayharmless_{args.gen_pref_model_name}_train_hh-rlhf-{args.task}{args.sft_obj}_database_split{i}.json",
            split="train",
        )
        scores_dataset.append(dataset["scores_sorted"])
        query_dataset.append(dataset["query"])
...
assert (
    np.shape(embedding_dataset)[1]
    == np.shape(scores_dataset)[1]
    == np.shape(query_dataset)[1]
)
...
for i in range(args.n_sample):
    ...
    idx_j_set = np.random.choice(
        args.n_sample, args.n_pairs, replace=False
    )  # select the second prompt randomly

    for j, idx_j in enumerate(idx_j_set):
        temp_selected_prompt_idx = np.random.choice(
            idx_set_list[idx_j], 1, replace=False
        ).item()
        if args.annotation_quality < 0:
            if (
                scores_dataset[temp_selected_prompt_idx][idx_j]
                > scores_dataset[idx_set_of_prompt_i[j]][i]
            ):
                positive_sample.append(
                    embedding_dataset[temp_selected_prompt_idx][idx_j]
                )
...
print(
    "mean of reward_diff:",
    np.mean(reward_diff_list),
    "stats",
    np.abs(reward_diff_list).mean(),
)
...
if args.rm_objective == "clf":
    for model_i in range(args.ensemble_number):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            embedding_dataset.numpy(),
            embedding_labels.numpy(),
            test_size=0.2,
            random_state=42 + model_i,
        )
        ...
        save_model(
            model,
            args.output_dir
            + f"/XPrompt_mlp_{args.rm_objective}_{model_i}_seed{args.seed}_firstn_{args.consider_first_n}_n_{args.n_sample}_pair_{args.n_pairs}_replacement_{args_replacement}_epoch{args.training_epochs}_lr{args.learning_rate}.ckpt",
        )
        print(
            f"Model saved as: {args.output_dir + f'/XPrompt_mlp_{args.rm_objective}_{model_i}_seed{args.seed}_firstn_{args.consider_first_n}_n_{args.n_sample}_pair_{args.n_pairs}_replacement_{args_replacement}_epoch{args.training_epochs}_lr{args.learning_rate}.ckpt'}"
        )
    ...
    for model_i in range(args.ensemble_number):
        ...
        np.random.seed(args.seed * 42 + model_i)
        X_train, X_test, y_train, y_test = train_test_split(
            siamese_dataset.numpy(),
            siamese_labels.numpy(),
            test_size=0.2,
            random_state=42 + model_i,
        )
