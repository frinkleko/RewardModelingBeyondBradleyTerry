...
def load_cluster_data(args):
    ...
    for i in range(10):
        ...
        if "helpful" in args.task:
            ds_path = (
                f"data_10split/rmmistral7b_{args.gen_pref_model_name}_train_"
                f"hh-rlhf-{args.task}{args.sft_obj_suffix}_database_split{i}.json"
            )
        else:
            ds_path = (
                f"data_10split/rayharmless_{args.gen_pref_model_name}_train_"
                f"hh-rlhf-{args.task}{args.sft_obj_suffix}_database_split{i}.json"
            )
...
if args.n_sample > n_prompts:
    print(
        f"Warning: n_sample ({args.n_sample}) > available prompts ({n_prompts}). Clamping."
    )
...
for i in range(args.n_sample):
    if args.consider_first_n > 0:
        idx_set_list.append(
            np.random.choice(
                n_splits, min(args.consider_first_n, n_splits), replace=False
            )
        )
...
p_list_fn = (
    f"/plist_XPrompt_mlp_{args.rm_objective}_seed{args.seed}_firstn_"
...
reward_diff_list_fn = (
    f"/reward_diff_list_XPrompt_mlp_{args.rm_objective}_seed{args.seed}_firstn_"
    f"{args.consider_first_n}_n_{args.n_sample}_pair_{args.n_pairs}_"
    f"replacement_{args_replacement}_epoch{args.training_epochs}_lr{args.learning_rate}.npy"
)
...
if args.rm_objective == "clf":
    for model_i in range(args.ensemble_number):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            embedding_dataset_tensor.numpy(),
        ...
        ckpt_name = (
            f"/XPrompt_mlp_{args.rm_objective}_{model_i}_seed{args.seed}_firstn_"
            f"{args.consider_first_n}_n_{args.n_sample}_pair_{args.n_pairs}_"
