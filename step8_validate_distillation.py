...
def main():
    ...
    for p_idx in tqdm(range(n_prompts)):
        ...
        _, i_corr = evaluate_model(teacher, embs, gt, is_isotonic=True, ir_model=ir_model)
    ...
    print(f"
Results (Mean Spearman Correlation):")
    print(f"Teacher ORM:         {np.nanmean(teacher_corrs):.4f}")
    print(f"Teacher + Isotonic:  {np.nanmean(iso_corrs):.4f} (Should be same as Teacher)")
    ...
    print("
Scale Check (First 5 samples of first prompt):")
